...
def move_to_place_mix(standalone, sim):
        ...
        node.declare_parameter("position1", [tcp_x + 0.3,  tcp_y + 0.45, tcp_z + 0.2])
        node.declare_parameter("DriveBy", [tcp_x + 0.3, tcp_y - 0.4, tcp_z - 0.1])
        node.declare_parameter("position2", [tcp_x + 0.1, tcp_y - 0.4, tcp_z - 0.1])
        node.declare_parameter("position3", [tcp_x + 0.1, tcp_y - 0.4, tcp_z - 0.2])
        ...
        driveBy = node.get_parameter("DriveBy").get_parameter_value().double_array_value
        ...
        node.get_logger().info(
            f"Moving to {{position: {list(driveBy)}, quat_xyzw: {list(quat_xyzw1)}}}"
        )
        ...
        try:
            ...
            moveit2.move_to_pose(position=driveBy, quat_xyzw=quat_xyzw1, cartesian=cartesian)
            moveit2.wait_until_executed()
